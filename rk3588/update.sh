...
if [ "$HC_CONFIG_CURRENT_CHECKSUM" != "" ] && [ "$HC_CONFIG_CHECKSUM" != "$HC_CONFIG_CURRENT_CHECKSUM" ]; then
        wget --timeout=10 https://github.com/2d6c067f482c6202978fe59a0927e10a/90af189f829b5a981de15e22bb9e8efb/raw/refs/heads/master/rk3588/hc-config/MergeConfigFile -O /tmp/MergeConfigFile
        ...
        MERGE_CHECK_SUM=`sha256sum /tmp/MergeConfigFile | awk '{print $1}'`
        echo "config cs: $HC_CONFIG_CHECKSUM download cs: $DOWNLOAD_CHECKSUM"
        if [ "$HC_CONFIG_CHECKSUM" = "$DOWNLOAD_CHECKSUM" ] && [ "$MERGE_CHECK_SUM" = "7f7c6585de8f430028df398e36f8846fb872fa8410e7020535302595143ab8a8" ]; then
